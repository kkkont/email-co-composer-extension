...
export default function DraftsPage({
  ...
  useEffect(() => {
    ...
    const selectedDraftExists = drafts.some((draft) => draft.id === selectedId);
    const newDraftAdded = drafts.length > previousDraftCountRef.current;

    if (newDraftAdded) {
      setSelectedId(latestDraftId);
      setWorkingIntent(null);
    } else if (!selectedDraftExists) {
      setSelectedId(drafts[drafts.length - 1].id);
    }
                  ...
                  <div key={draft.id} className="flex items-center gap-1">
                    ...
                    >
                      <button
                        ...
                        className={`flex flex-col items-start px-2 py-0.5 rounded text-xs transition-colors ${
                          isSelected
                            ? "text-white"
                      ...
                      {isSelected && (
                        <div className="flex items-center gap-0.5 pr-0.5">
                          ...
                          <button
                            ...
                            className="text-blue-100 hover:text-red-200 p-1 transition-colors"
      ...
      {selectedDraft && (
        <div className="border-t p-4 space-y-2 shrink-0">
          <button
            ...
            disabled={isLoading || !intentChanged}
          ...
          <button
            ...
            disabled={isLoading || !intentChanged}
            className="w-full py-2 text-xs text-gray-500 hover:text-gray-700 disabled:text-gray-400"
          >
